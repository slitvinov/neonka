...
static void walk(int32_t *pR, int32_t *pN, int32_t *cR, int32_t *cN,
                 int (*diff)(int32_t, int32_t), int64_t *bins) {
  int i = 0, j = 0;
  int32_t dn, d, dist;
...
int main(int argc, char **argv) {
  ...
  if (bins == NULL) {
    fprintf(stderr, "tp.c: error: calloc failed\n");
    return 1;
  }
  while (fread(&prev, sizeof prev, 1, stdin) == 1 &&
         fread(&cur, sizeof cur, 1, stdin) == 1) {
    walk(prev.aR, prev.aN, cur.aR, cur.aN, diff_ask, bins);
    walk(prev.bR, prev.bN, cur.bR, cur.bN, diff_bid, bins);
  }
  int i;
  for (i = 0; i < NB; i++)
    if (bins[i])
